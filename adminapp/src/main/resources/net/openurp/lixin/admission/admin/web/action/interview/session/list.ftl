[@b.head title = "面试时段选择系统－管理员"/]
  [@b.grid items=sessions var="session"]
    ...
    [@b.row]
      [@b.boxcol/]
      [@b.col title="批次" property="batch.name" width="30%"]${session.batch.name}[/@]
      [@b.col title="专业" property="major.name"/]
      [@b.col title="开始时间" property="beginOn"]${session.beginAt?string("yyyy-MM-dd HH:mm")}[/@]
      [@b.col title="结束时间" property="endOn"]${session.endAt?string("yyyy-MM-dd HH:mm")}[/@]
